#! python3
# From the shell, this script takes a CSV file (as the first argument, 'SOMECSV.csv') and an output name string without an extension (as the second argument, 'EXAMPLE'), eliminates duplicate names, and returns both (1) an output file with duplicates stripped, and (2) an output file of possible duplicates left in the main file. Names in (2) are flagged 'Check me' in (1). Sample command: py dupeRemove.py 'names.csv' 'example'.
        ...
        if len(itemTwo[1][4]) > 1:
            ...
            if itemOneSurname == itemTwoSurname:
                ...
                try:
                    ...
                    itemTwoMiddle = ''
                    itemTwoMI = ''
                if itemOneFirst in itemTwoFirst and itemOneMiddle in itemTwoMiddle:
                    return True
                elif itemOneFI == itemTwoFI and itemOneMI == itemTwoMI:
                    return True
                elif itemTwoMiddle == itemOneFirst and itemOneMiddle == '':
                    return True
    return False
        ...
        if itemTwoPub >= 2:
            ...
            if surnameDist <= 2:
                if itemOneFirst in itemTwoFirst:
                    if itemOneMiddle != '' and itemTwoMiddle != '' and itemOneMiddle in itemTwoMiddle:
                        return True
